...
void FIFO()
{
  ...
  while (true)
  {
    ...
    for (int i_cores = 0; i_cores < NUM_CORES; i_cores++)
    {
      ...
      for (int i_proccess = 0; i_proccess < NUM_PROCCSSES; i_proccess++)
      {
        ...
        if (process->core == i_cores + 1 && !process->isExecuted)
        {
          ...
          else if (process->arrivalTime < processes[proccessIndex].arrivalTime)
            proccessIndex = i_proccess;
        }
      }
    ...
    for (int i_proccess = 0; i_proccess < NUM_PROCCSSES; i_proccess++)
    {
      ...
      {
        /// @bug If a process is running and requested and IO, all processes should wait it
        ///      There is no process will take the core if the earlier process made an IO
        ///
        ///      We are aware that this is not what DOCTOR asked, but we see this is more realistic
        ///
        ///      FIFO is beign discuessed before taking IO corruption into consideration, and if a process
        ///      asked for IO, its state should change. and letting another one take the core
        ///      is a design we do not believe in
        ///
        ///      Each process has its state and context, when asking for IO it should preserve its state
        ///      in order to complete its proccessing after IO. and by making another process takes the core
        ///      I am breaking the Schedular Algorism for managing First arrived process imo.
        ///
        ///      And by doing such as thing, this means that the process leaves the core (In this scheduling algorism)
        ///      And updating the arrivalTime of it after it came back form the IO is logical fault in real life.
        ///
        ///      Since the DOCTOR may not be persuaded by these words, remove the next line and it works the way
        ///      she wants it to.
