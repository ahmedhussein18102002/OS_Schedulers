  ...
  while (true)
  {
    ...
    for (int i_cores = 0; i_cores < NUM_CORES; i_cores++)
    {
      ...
      for (int i_proccess = 0; i_proccess < NUM_PROCCSSES; i_proccess++)
      {
        ...
        if (process->core == i_cores + 1 && !process->isExecuted)
        {
          ...
          if (proccessIndex > -1 && processes[proccessIndex].priority < process->priority)
            proccessIndex = i_proccess;

          if (
              proccessIndex > -1 &&
              processes[proccessIndex].priority == process->priority &&
              processes[proccessIndex].arrivalTime > process->arrivalTime)
          {
            proccessIndex = i_proccess;
          }
        }
    ...
    for (int i_proccess = 0; i_proccess < NUM_PROCCSSES; i_proccess++)
    {
      if (processes[i_proccess].io_step)
      {
        ...
        /// @bug If the proccess is running IO and highest prioritized one come to the core.
        ///      It will wait the less priority process to complete its IO before taking the core
        ///      from it.
        ///
        ///      We don't know how Priority should work in this situation, and we don't have a simulaton
