...
/// @details If all proccesses have the same Stride it will work as FIFO (picks the first one arrived)
void Stride()
{
  print_line('=');
    ...
    for (int i_cores = 0; i_cores < NUM_CORES; i_cores++)
    {
      ...
      for (int i_proccess = 0; i_proccess < NUM_PROCCSSES; i_proccess++)
      {
        ...
        if (process->core == i_cores + 1 && !process->isExecuted)
        {
          ...
          if (process_stride_pass(process) < process_stride_pass(&processes[proccessIndex]))
            proccessIndex = i_proccess;
